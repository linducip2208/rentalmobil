...
class PublicBookingController extends Controller
{
    ...
    {
        return view('booking.index', ['vehicles' => Vehicle::with(['category', 'brand'])->where('is_active', true)->where('status', 'available')->paginate(12), 'locations' => Location::active()->get(), 'addons' => Addon::active()->get()]);
    }
...
return response()->json(['hold_token' => $hold->hold_token, 'expires_at' => $hold->expires_at->toIso8601String(), 'minutes_left' => now()->diffInMinutes($hold->expires_at)]);
    ...
    {
        $d = $r->validate(['name' => 'required|string|max:150', 'email' => 'required|email|max:150', 'phone' => 'required|string|max:30', 'vehicle_id' => 'required|exists:vehicles,id', 'pickup_location_id' => 'required|exists:locations,id', 'return_location_id' => 'nullable|exists:locations,id', 'start_date' => 'required|date|after_or_equal:today', 'end_date' => 'required|date|after:start_date', 'rental_type' => 'required|in:self_drive,with_driver,airport_transfer,corporate', 'addon_ids' => 'nullable|array', 'addon_ids.*' => 'integer|exists:addons,id', 'promo_code' => 'nullable|string|max:50', 'notes' => 'nullable|string|max:1000', 'session_id' => 'nullable|string|max:64', 'referral_code' => 'nullable|string|max:32', 'group_booking_id' => 'nullable|exists:group_bookings,id']);
        ...
        try {
            ...
            // Konflik dengan hold pemesan lain → 409 agar frontend menampilkan pesan ramah.
            if (str_contains($e->getMessage(), 'di-hold') || str_contains($e->getMessage(), 'not available')) {
                return response()->json(['message' => $e->getMessage()], 409);
            }throw $e;
        }

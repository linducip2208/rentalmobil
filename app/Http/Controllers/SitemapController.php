...
class SitemapController extends Controller
{
    ...
    public function index(): Response
    {
        $xml = Cache::remember('sitemap:index', 86400, function () {
            ...
            for ($page = 1; $page <= $customPages; $page++) {
                $chunks[] = ['section' => 'custom', 'page' => $page, 'lastmod' => SitemapEntry::active()->max('updated_at') ?? now()];
            }
    ...
    private function buildChunk(string $section, int $page): ?string
    {
        ...
        if ($section === 'core') {
            ...
            foreach (['/' => ['weekly', '.80'], '/docs' => ['monthly', '.60'], '/blog' => ['weekly', '.70'], '/faq' => ['monthly', '.60'], '/contact' => ['monthly', '.50'], '/sewa-mobil' => ['weekly', '.80']] as $path => [$freq,$priority]) {
                $entries[] = [$base.$path, $freq, $priority, null];
            }
            ...
            Category::active()->get()->each(function ($category) use (&$entries, $base) {
                $slug = Str::slug($category->name);
                $entries[] = [$base.'/best-'.$slug, 'monthly', '.65', $category->updated_at];
                $entries[] = [$base.'/best-'.$slug.'-'.now()->year, 'monthly', '.65', $category->updated_at];
            });
            $vehicles->take(250)->each(function ($a) use ($vehicles, &$entries, $base) {
                $vehicles->where('id', '>', $a->id)->take(4)->each(fn ($b) => $entries[] = [$base.'/compare/'.$a->slug.'-vs-'.$b->slug, 'monthly', '.60', max($a->updated_at, $b->updated_at)]);
            });
        } else {
            ...
            foreach ($rows as $row) {
                $entries[] = [$base.'/'.ltrim($row->url, '/'), $row->change_frequency ?? 'monthly', (string) ($row->priority ?? '.50'), $row->last_mod];
            }
        }

...
class GpsIntelligenceService
{
    public function evaluate(GpsLog $log): void
    {
        ...
        if ($previous && $previous->recorded_at->diffInSeconds($log->recorded_at) <= 120) {
            ...
            if ($delta > $threshold) {
                $this->record($log, $driverId, 'harsh_acceleration', 2, ['delta' => $delta]);
            }
            if ($delta < -$threshold) {
                $this->record($log, $driverId, 'harsh_brake', 2, ['delta' => $delta]);
            }
        }
        foreach (GpsGeofence::where('is_active', true)->get() as $geofence) {
            ...
            if ($violation) {
                $this->record($log, $driverId, 'geofence_violation', 4, ['geofence_id' => $geofence->id, 'geofence' => $geofence->name, 'rule' => $geofence->type]);
            }
        }
    }
    ...
    private function contains(array $geometry, float $latitude, float $longitude): bool
    {
        ...
        for ($i = 0, $j = count($points) - 1; $i < count($points); $j = $i++) {
            ...
            if ((($yi > $latitude) !== ($yj > $latitude)) && ($longitude < ($xj - $xi) * ($latitude - $yi) / (($yj - $yi) ?: 0.0000001) + $xi)) {
                $inside = ! $inside;
            }
        }

...
class GpsCommandService
{
    ...
    {
        if (! in_array($reviewer->role, ['super_admin', 'admin', 'manager'], true)) {
            throw new \RuntimeException('Role ini tidak berwenang menyetujui perintah GPS.');
        }
        if ($command->requested_by === $reviewer->id) {
            throw new \RuntimeException('Pemohon tidak boleh menyetujui perintahnya sendiri.');
        }
        if ($command->status !== 'pending_approval') {
            throw new \RuntimeException('Perintah sudah diproses.');
        }
    ...
    {
        if (! in_array($reviewer->role, ['super_admin', 'admin', 'manager'], true)) {
            throw new \RuntimeException('Role ini tidak berwenang menolak perintah GPS.');
        }
    ...
    public function send(GpsCommand $command): GpsCommand
    {
        if ($command->status !== 'approved') {
            throw new \RuntimeException('Hanya perintah yang disetujui dapat dikirim.');
        }
        $integration = $command->tracker->integration;
        $provider = $integration?->provider;
        if (! $integration || ! $provider || blank($provider->base_url) || blank($integration->commands_endpoint)) {
            throw new \RuntimeException('Konfigurasi endpoint perintah tidak lengkap.');
        }
        ...
        if ($integration->auth_type === 'bearer') {
            ...
            $request = $request->withBasicAuth((string) $name, (string) $secret);
        } elseif ($integration->auth_type === 'header') {
            $request = $request->withHeader($name, (string) $secret);
        }

        $payload = ['device_id' => $command->tracker->external_device_id, 'command' => $command->command_name, 'parameters' => $command->parameters ?? []];
        if ($integration->auth_type === 'query') {
            $payload[$name] = $secret;
        }

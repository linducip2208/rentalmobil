...
use App\Services\Gps\GpsAdapterManager;
    ...
    public static function configure(Table $table): Table
    {
        return $table
            ->columns([
                ...
                TextColumn::make('health_status')->label('Health')->badge()->color(fn ($state) => match ($state) {
                    'healthy' => 'success', 'degraded' => 'warning', 'down' => 'danger', default => 'gray'
                }),
            ...
            ->recordActions([
                Action::make('test')->label('Tes koneksi')->icon('heroicon-o-signal')->action(function ($record) {
                    try {
                        $result = app(GpsAdapterManager::class)->for($record)->test($record);
                        Notification::make()->title($result['message'])->color($result['ok'] ? 'success' : 'danger')->send();
                    } catch (\Throwable $e) {
                        Notification::make()->title('Koneksi gagal')->body($e->getMessage())->danger()->send();
                    }
                ...
                Action::make('sync')->label('Sinkronkan')->icon('heroicon-o-arrow-path')->action(function ($record) {
                    try {
                        $r = app(GpsSyncService::class)->sync($record);
                        Notification::make()->title("{$r['saved']} posisi disimpan")->success()->send();
                    } catch (\Throwable $e) {
                        Notification::make()->title('Sinkronisasi gagal')->body($e->getMessage())->danger()->send();
                    }
                })->visible(fn ($record) => in_array($record->adapter_format, ['rest_polling', 'traccar_compatible'], true)),
                EditAction::make(),
            ])

...
<head>
    ...
    <title>{{ $seoTitle ?? config('app.name') . ' — Sewa Mobil Terpercaya' }}</title>
    <meta name="description" content="{{ $seoDescription ?? 'Sewa mobil terpercaya di Indonesia. Armada lengkap, harga transparan, booking online.' }}">
    @if($seoCanonical ?? null)
    <link rel="canonical" href="{{ $seoCanonical }}">
    @endif
    <meta property="og:title" content="{{ $seoTitle ?? config('app.name') }}">
    <meta property="og:description" content="{{ $seoDescription ?? 'Sewa mobil terpercaya di Indonesia' }}">
    ...
    <meta name="twitter:description" content="{{ $seoDescription ?? 'Sewa mobil terpercaya di Indonesia' }}">
    @if($seoJsonLd ?? null)
    ...
    <style>
        ...
        .card-lift:hover { transform: translateY(-4px); box-shadow: 0 12px 32px -8px rgba(0,0,0,.12); }
        @media (prefers-reduced-motion: reduce) {
            .reveal, .card-lift { animation-duration: 0.01ms !important; transition-duration: 0.01ms !important; }
        }
    </style>
    @stack('head')
</head>

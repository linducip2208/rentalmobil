...
<header class="sticky top-0 z-30 border-b border-slate-200 bg-white/90 backdrop-blur-xl">
    <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        @auth('customer')
        <nav class="flex items-center gap-2 text-sm"><a class="hidden rounded-lg px-3 py-2 hover:bg-slate-100 sm:block" href="{{ route('portal.orders') }}">Pesanan</a><a class="hidden rounded-lg px-3 py-2 hover:bg-slate-100 sm:block" href="{{ route('portal.invoices') }}">Invoice</a><form method="post" action="{{ route('portal.logout') }}">@csrf<button class="min-h-10 rounded-lg border border-slate-300 px-3 font-semibold hover:bg-slate-50">Keluar</button></form></nav>
        @endauth
    </div>
</header>
